...
/*   Updated: 2023/05/06 00:19:22 by sciftci          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../cub3d.h"

static t_render	*render_new(void)
{
	t_render	*element;
...
static int get_texture_pixel(t_img img, t_render *render)
{
	int		tx;
	int		ty;
	double	ty_step;
...
static void draw_pixel(t_vars *vars, t_render *render, int x, int y)
{
	t_img	*img;
	int		color;
...
static void	get_wall_dims(t_vars *vars, t_render *render,
	t_coor start_pos, double degree)
{
	render->degree = degree;
	render->hit_wall = get_hit_wall(vars, start_pos,
...
void	render_scene(t_vars *vars)
{
	...
	while (degree <= FOV)
	{
		while (!scene|| (scene && vars->map.map[(int)((t_render *)(scene->strct))->hit_wall.y][(int)((t_render *)(scene->strct))->hit_wall.x] != WALL))
		{
			...
			if (!scene)
				get_wall_dims(vars, temp, vars->player.pos, degree);		
			else
				get_wall_dims(vars, temp, ((t_render *)(scene->strct))->hit_wall, degree);								
			list_add_front(&scene, list_new(temp));
		}
		draw_walls(vars, scene);
		list_clear(&scene);
		degree += DEG_INC;
	}
}
